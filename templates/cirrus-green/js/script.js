jQuery(document).ready(function ($) {
    ...
    $(window).on('resize', function () {
        ...
        $('.menuresp').css('top', $('#header_wrap').outerHeight() - 1);
    ...
    $('#clientes').css('padding-top', $('#header_wrap').height());
    $('#clientes').css('margin-top', -($('#header_wrap').height()));

    $(window).on('resize', function () {
        $('.blog-pattern .img-intro').height(parseInt($('.blog-pattern .img-intro').width() * 0.75));
    }).trigger('resize');
